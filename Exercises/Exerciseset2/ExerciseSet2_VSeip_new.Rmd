...
suppressWarnings(source("C:/Users/Vanessa Seip/Dropbox/Privat/MBA/INSEAD/Academics/P3/Big Data Analytics/RStudio/INSEADAnalytics/AnalyticsLibraries/library.R"))
...
getwd()
setwd("C:/Users/Vanessa Seip/Dropbox/Privat/MBA/INSEAD/Academics/P3/Big Data Analytics/RStudio/INSEADAnalytics_VSeip/Exercises/Exerciseset2")
list.files()
```
...
* Q1: 6 Components; with those we explain 52.29% of the variance

* Q2: The highest weight is 14.04 (Eigenvalue)

* Q3: Component 1: US Treasury Bonds, Comp.2: European Currencies, Comp.3: European Stock indixes, Comp. 4: US Stock Indexes, Comp. 5: Euro bonds, Comp. 6:  Oil prices, Comp. 7: Industrual metal prices, Comp. 8: Agricultural commodities, Comp.9: Pressures metal prices, Comp.10: Asian Stock Market indexes

* Q4a: Pre-Crisis
```{r echo=TRUE}
pre_crisis_data <- futures_data[as.Date(rownames(futures_data)) < "2007-10-01" & as.Date(rownames(futures_data)) > "2002-11-01",]
Variance_Explained_Table_results_pre_Crisis<-PCA(pre_crisis_data, graph=FALSE)
Variance_Explained_Table_pre_Crisis<-Variance_Explained_Table_results$eig
colnames(Variance_Explained_Table_pre_Crisis)<-c("Eigenvalue", 
  "Pct of explained variance", "Cumulative pct of explained variance")
...
```{r}
df <- cbind(as.data.frame(eigenvalues_pre_crisis), c(1:length(eigenvalues_pre_crisis)), rep(1,length(eigenvalues_pre_crisis)))
colnames(df) <- c("eigenvalues", "components", "abline")
iplot.df(melt(df, id="components"))
```

```{r echo=TRUE, tidy=TRUE}
corused_pre_crisis = cor(pre_crisis_data[,apply(pre_crisis_data!=0,2,sum) > 10, drop=F])
Rotated_Results_pre_crisis<-principal(corused_pre_crisis, nfactors=20, rotate="varimax",score=TRUE)
Rotated_Factors_pre_crisis<-round(Rotated_Results_pre_crisis$loadings,2)
Rotated_Factors_pre_crisis<-as.data.frame(unclass(Rotated_Factors_pre_crisis))
colnames(Rotated_Factors_pre_crisis)<-paste("comp",1:ncol(Rotated_Factors_pre_crisis),sep=" ")

sorted_rows_pre_crisis <- sort(Rotated_Factors_pre_crisis[,1], decreasing = TRUE, index.return = TRUE)$ix
Rotated_Factors_pre_crisis <- Rotated_Factors_pre_crisis[sorted_rows,]
Rotated_Factors_pre_crisis[abs(Rotated_Factors_pre_crisis) < 0.3]<-NA
```

```{r}
show_data_pre_crisis <- Rotated_Factors_pre_crisis 
iprint.df(show_data_pre_crisis, scale=TRUE)
#print(show_data_pre_crisis)
```
...
Variance_Explained_Table_Crisis<-Variance_Explained_Table_results_Crisis$eig
...
```{r}
df           <- cbind(as.data.frame(eigenvalues_crisis), c(1:length(eigenvalues_crisis)), rep(1, length(eigenvalues_crisis)))
colnames(df) <- c("eigenvalues", "components", "abline")
iplot.df(melt(df, id="components"))
```
...
Rotated_Results_crisis<-principal(corused_crisis, nfactors=20, rotate="varimax",score=TRUE)
...
colnames(Rotated_Factors_crisis)<-paste("comp",1:ncol(Rotated_Factors_crisis),sep=" ")
...
iprint.df(show_data_crisis, scale=TRUE)
```
...
* Q5: Returns are more correlated during the crisis than before the crisis. Component 1 explains much more of the variance than before. Only 4 instead of 6 components needed to explain 50% of the variance. The value of the futures on the US Treasury market is more strongly correlated. Pre-Crisis Data is very similar to the full data set from Q1-3.
...
* Q1:
* The first signal used is 0
* Iteration is done for 13 steps ((250-10)/20+1)
* For each step a signal (+/-, long or short) is applied
* When the outcome gives a + signal on average, you buy, otherwise you sell 
* Constraint: invest 1$ every day
* After that the weights of the different futures are calculated, with total weight = 100%

* Q2:
* I increased the number of iterations in order to make the result more "accurate"/ the probability of a good output higher
* I also increased the time between the decisions from 2 to 4 in order to make decisions more stable

```{r echo=TRUE, tidy=TRUE}
signal_used = 0*futures_data # just initialize the trading signal to be 0
# Take many moving Average (MA) Signals and let them "vote" with their sign (+-1, e.g. long or short vote, for each signal)
MAfreq<-seq(10,300,by=5)
for (iter in 1:length(MAfreq))
  signal_used = signal_used + sign(apply(futures_data,2, function(r) ma(r,MAfreq[iter])))
# Now make sure we invest $1 every day (so the sum of the absolute values of the weights is 1 every day)
signal_used = t(apply(signal_used,1,function(r) {
...
colnames(signal_used) <- colnames(futures_data)
# Now create the returns of the strategy for each futures time series
strategy_by_future <- scrub(shift(signal_used,4)*futures_data) # signal 2 days ago
# finally, this is our futures trend following strategy
trading_strategy = apply(strategy_by_future,1,sum)
names(trading_strategy) <- rownames(futures_data)
```
