...
**Your Answers here:**
<br> Q1. In dataSet1.R the vector mytickers = c("SPY", "AAPL") is used to run a loop first on "Spy" and on "AAPL" afterwards

<br> Q2. 
<br> AAPL = StockReturns[,"AAPL"]
<br> round(100*sum(AAPL), 1) #597.2
<br> round(100*mean(AAPL),3) #0.148
<br> round(100*sd(AAPL),3) #2.388

<hr> Q3.
<hr> add YHOO to the dataSet.R and run source("dataset1.R) again
<hr> YHOO = StockReturns[,"YHOO"]
<hr> round(100*sum(YHOO), 1) #290.7
<hr> round(100*mean(YHOO),3) #0.072
<hr> round(100*sd(YHOO),3) #3.018
...
**Your Answers here:**
<br> nrow(transposedData) #3
<br> ncol(transposedData) #4035
<br> note: table includes dataset of YHOO

<br> portfolio_transpose = apply(transposedData,2,mean)
<br> pnl_plot(portfolio_transpose)

```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
transposedData = t(StockReturns)
portfolio_transpose = apply(transposedData,2,mean)
names(portfolio_transpose) <- colnames(transposedData)
...
**Your Answers here:**
<br> Q1.
<br> The Dataset already starts at 2001 - startDate = "2001-01-01" - I assume it whould have startet at 2005-01-01
<br> In this case, I would have changed the start date from "2005-01-01" to "2001-01-01"
<br> I would then redo all exercises with the updated dataset

<hr> Q2.
<hr> Step 1: Decide on start date, e.g. 2005-01-01
<hr> Step 2: Decide on companies, e.g. Mictosoft (MSFT) and Nike (NKE)
<hr> Step 3: Change start date and vector mytickers to c("MSFT", "NKE") in the dataset and run again source("dataset1.R")
<hr> Step 4: Portfolio_new = apply(StockReturns,1,mean)
<hr> Step 5: pnl_plot(Portfolio_new)

```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
portfolio_new = apply(StockReturns,1,mean)
names(portfolio_new) <- rownames(StockReturns)
...
**Your Answers here:**
<br> I changed the dataset back to use data from SPY and AAPL, starting 2005-01-01
<br> sum(myData != StockReturns[1:20,]) gives the result 20
<br> Running myData + StockRetruns[1:40,] gives an error "Error in FUN(left, right) : non-numeric argument to binary operator"
<br> because the dimensions of the tables are not the same
